...
In the **R** chunk below, we will load in a picture stored as `soton.png` in our main directory.  We then give it the caption of "Southampton logo", the label of "reedlogo", and specify that this is a figure.  Make note of the different **R** chunk options that are given in the R Markdown file (not shown in the knitted document).

```{r reedlogo, fig.cap="Southampton logo"}
include_graphics(path = "figure/soton.png")
```
